 ...
 * via commit c5dbb89fc2ac("bpf: Expose bpf_get_socket_cookie to tracing
 * programs"). Since the helper is used to provide a unique socket identifier,
 * we could use the sock itself as the identifier if the helper is not
 * available. Here, we use BPF_FUNC_check_mtu to check the availability of the
 * helper since they are both introduced in v5.12.
 *
 * see:
...
/**
 * During kernel 6.6 development cycle, several bitfields in struct inet_sock
 * gone, they are placed in inet_sock::inet_flags instead ([0]).
 *
 * References:
 *   [0]:
 * https://lore.kernel.org/all/20230816081547.1272409-1-edumazet@google.com/
 */
